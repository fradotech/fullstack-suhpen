...
export class InventoryAction {
  static useIndex(
    ...
    const fetch = async () => await API.get(Path.inventory.index, req)
  ...
  static async create(
    ...
    data = dto(data)
    const res = await API.post(Path.inventory.index, data)
  ...
  static async findOne(id: string): Promise<IApiRes<InventoryResponse>> {
    const res: IApiRes<InventoryResponse> = await API.get(Path.inventory.id(id))
